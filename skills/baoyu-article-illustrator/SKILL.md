...
description: Analyzes article structure, identifies positions requiring visual aids, generates illustrations with Type × Style two-dimension approach. Use when user asks to "illustrate article", "add images", "generate images for article", or "为文章配图". 关键词：文章配图、插图生成。
---
...
**Image Delivery**: After generating each image, send it to the user:

```
message(action="send", filePath="/path/to/output.png", message="<caption>")
```
