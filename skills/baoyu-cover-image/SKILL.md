...
description: Generates article cover images with 5 dimensions (type, palette, rendering, text, mood) combining 9 color palettes and 6 rendering styles. Supports cinematic (2.35:1), widescreen (16:9), and square (1:1) aspects. Use when user asks to "generate cover image", "create article cover", or "make cover". 关键词：封面图、文章封面、生成封面、做个封面图。
---
...
**Image Generation**: Use the nano-banana-pro skill to generate images:

```bash
uv run {nano-banana-pro-baseDir}/scripts/generate_image.py --prompt "<prompt>" --filename "<output>.png" --resolution 2K
```

**Image Delivery**: After generating the image, send it to the user:

```
message(action="send", filePath="/path/to/cover.png", message="<caption>")
```

Do NOT include file paths as text in your reply. Always use the message tool with filePath to deliver images.
