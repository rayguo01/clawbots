...
export function formatThreadMarkdown(thread: unknown, options: ThreadMarkdownOptions = {}): string {
  ...
  if (includeHeader) {
    ...
    const sourceUrl =
      options.sourceUrl ?? buildTweetUrl(username, candidate.rootId ?? candidate.requestedId);
    if (sourceUrl) {
