...
description: |
  ...
  关键词：发布帖子、社交媒体、排期发布、定时发布、发推特、发LinkedIn、内容日历。
metadata:
  openclaw:
    requires:
      bins: ["uv"]
      env: ["POSTIZ_URL", "POSTIZ_EMAIL", "POSTIZ_PASSWORD"]
---
...
```bash
# Single platform
uv run {baseDir}/scripts/post.py \
  --platform x \
...
```bash
# Check last 30 days
uv run {baseDir}/scripts/check_duplicates.py

# Check last 7 days
uv run {baseDir}/scripts/check_duplicates.py --days 7

# Check if specific content would be duplicate
uv run {baseDir}/scripts/check_duplicates.py --content "Your proposed post content"
```
