...
def search_xiaohongshu(query: str, max_results: int = 10, region: str = 'cn-zh') -> list:
    """搜索小红书探店笔记"""
    ...
    for sq in search_queries:
        try:
            results = ddgs.text(sq, max_results=max_results, region=region)
            for r in results:
                url = r.get('href', '')
...
def main():
    ...
    parser.add_argument('--region', default='cn-zh', help='搜索区域 (cn-zh, sg-en, wt-wt 等)')
    parser.add_argument('--json', action='store_true', help='JSON输出')
    args = parser.parse_args()

    notes = search_xiaohongshu(args.query, args.max, region=args.region)
